...
class _CropScreenState extends State<CropScreen> {
  ...
  Future<void> _confirm() async {
    ...
    // Image.file(fit: BoxFit.contain) fills the FULL screen (Stack expand)
    final viewW = viewSize.width;
    final viewH = viewSize.height;
    final imageAspect = _imageSize.width / _imageSize.height;
    final viewAspect = viewW / viewH;

    // BoxFit.contain display size (at scale=1)
    double fitW, fitH;
    if (imageAspect > viewAspect) {
      fitW = viewW;
      fitH = viewW / imageAspect;
    } else {
      fitH = viewH;
      fitW = viewH * imageAspect;
    }

    // After user scale (Transform.scale centers on widget center)
    final displayW = fitW * _scale;
    final displayH = fitH * _scale;

    // Image center = screen center, then offset by user drag
    final displayLeft = (viewW - displayW) / 2 + _offset.dx;
    final displayTop = (viewH - displayH) / 2 + _offset.dy;

    // Screen coords → image pixel coords
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        fit: StackFit.expand,
        children: [
          // Image layer - GestureDetector must cover full screen
          GestureDetector(
          ...
          // Crop overlay (must not block gestures)
          if (_imageLoaded)
            IgnorePointer(
              child: LayoutBuilder(
